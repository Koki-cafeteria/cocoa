...
class mg_hsc_y3_cosmic_shear(_cosmolike_prototype_base):
  def initialize(self):
    super(mg_hsc_y3_cosmic_shear,self).initialize(probe="xi")
        ...
        ci.compute_data_vector_masked_with_baryon_pcs(
          Q = [
                params_values.get(p, None) for p in [
                  "hsc_BARYON_Q"+str(i+1) for i in range(self.npcs)
                ]
              ]
        )
      )
